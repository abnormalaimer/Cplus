...
int main() {
    ...
    int days;
    cout<<"The first is: "<<d1.toString()
    <<"\nToday's date is: "
    <<d2.toString("ddd MMM d,yyyy")<<Qt::endl;

    if (d1<d2)
        cout<<d1.toString("MM/dd/yy")<<"is earlier than "
        <<d2.toString("yyyyMMdd")<<Qt::endl;

    cout<<"There are "<<d1.daysTo(d2)
    <<"days between "
    <<d1.toString("MMM dd,yyyy")<<" and "
    <<d2.toString(Qt::ISODate)<<Qt::endl;
    ...
    cout<<"The first date was"<<d1.toString()
            ...
            << d1.addDays(days).toString()<<Qt::endl;
    cout<<"First date displayed in longer format: "
    <<d1.toString("dddd,MMMM dd,yyyy")<<Qt::endl;
}
